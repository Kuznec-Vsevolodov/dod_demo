...
io.on('connection', socket => {
    ...
    socket.on('joinRoom', ({ username, room, avatar, game_type }) => {
      ...
      socket.broadcast.to(user.room).emit(
        'message', 
        formatMessage(botName, `A ${user.username} has joined the chat`, user.room, "chatbot.png", false)
      );
    });
    ...
    socket.on('disconnect', () => {
      ...
      if(user){
        io.to(user.room).emit('message', formatMessage(botName, `A ${user.username} has left the chat`, 'chatbot.png', false));
      }
...
app.post('/sing-queue', function(req, res){
  ...
  sing_queue_collection.findOne(function(err, data){
        if (err){
          return res.sendStatus(500);
        }
        if(data == null){
